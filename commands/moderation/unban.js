...
module.exports = class UnBanCommand extends Commando.Command {
  constructor(client) {
    super(client, {
      ...
      examples: ["unban hacks4life apologized for spam"],
      guildOnly: true,
      args: [{
          key: 'user',
  ...
  run(msg, {
    ...
    if (msg.guild.ownerID === msg.member.id) {
      ...
      msg.guild.unban(user, reason).then(bannedUser => {
        msg.reply(`Unbanned ${bannedUser} for ${reason}.`);
      }).catch(err => {
        msg.reply(`The user could not be unbanned.`);
        console.log(err);
      });
